...
public class UriEncoding {
	 ...
	 *            (e.g., if the input is already (partly) in URI/IRI format, and
	 *            you don't want slashes to be encoded). This should be used
	 *            carefully: if the string is already encoded, some sequences
	 *            might end up double-encoded.
	 ...
	 * Resource Identifiers (IRIs, RFC3987)</a> syntax, where characters outside
	 * the ASCII range can appear unencoded. The encoding is otherwise the same
	 * as for URIs / RFC3986.
	 * 
	 * <p>
	 ...
	 *            might end up double-encoded.
	 * @param javaEncoding
	 *            If true, also encode characters that would be disallowed by
	 *            Java's URI class
	 * 
	 * @return A %-encoded version of the string
	 ...
	 * 
	 * <p>
	 * Note that this decoder will fail if the encoded octets are not valid
	 * UTF-8
	 * 
	 * @param s
	 *            A %-encoded string
	...
	public static String[] checkCharacters(int first, int last) {
		...
		for (int i = first; i <= last; i++) {
			...
			try {
				URI uri = new URI("http://example.com/" + s);
				if (firstOk == -1)
					firstOk = i;
				if (firstBad != -1) {
